...
Sometimes which algorithm or which best special-case network is surprising. For instance, selecting the top-3 elements in a size-9 array is best done by adapting Hibbard algorithm, even though there is a special best (by comparators) network for size 9:
